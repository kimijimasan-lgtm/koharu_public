...
function compareTransportRoutes(stationName, destName, departTimeStr = '10:00') {
  ...
  const allPrefectures = Object.keys(PREFECTURE_STATIONS);
  ...
  for (const pref of allPrefectures) {
    ...
    for (const station of stations) {
      for (const destKey of allDestinations) {
        const dest = DESTINATIONS[destKey];
        // We only test Hokkaido for now since it's the current context, but let's test all
        const destName = dest.name;
        
        try {
           const route = compareTransportRoutes(station, destName, '10:00');
           const rec = route.recommended;
           const details = route[rec];
           
           if (!details) {
              anomalies.push({ station, dest: destName, error: 'No route generated' });
              continue;
           }
           
           if (details.time > 600) { // More than 10 hours
              anomalies.push({ station, dest: destName, time: details.time, type: rec });
           }
        } catch(e) {
           anomalies.push({ station, dest: destName, error: e.message });
        }
      }
    }
  }
